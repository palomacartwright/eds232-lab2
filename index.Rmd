...
k-means clustering specifies the number of clusters needed. The algorithm randomly assigns each observation to a cluster and finds the centroid of each cluster. Then the algorithm: - Reassigns data points to the cluster whose centriod is closest - Calculates the new centriod of each cluster.
...
What are the rows and columns composed of in the dune data frame? The rows in the `dune` data frame are sites and the columns are individual species.
